...
namespace UnitTests
{
    ...
    public class AccelerationTests
    {
        ...
        [DataRow(95, 0, 0, 545)]
        [DataRow(100, 0, 0, 598)]
        public void AccelerationTest_WhenHost_Approaching_Stopped_Lead_With_Ample_Distance_Host_Should_Accelerate(int hostMph, int hostX, int leadCarMph, int leadCarX)
        {
        ...
        public void AccelerationTest_WhenHost_Approaching_Lead_With_Ample_Distance_Host_Should_Accelerate(int hostMph, int hostX, int leadCarMph, int leadCarX)
        {
            double updateIntervalTotalMilliseconds = 250;
            Constants constants = new Constants();
            var SUT = Vehicle.Factory.Create("host car", hostMph, hostX, 1, true);
        ...
        public void AccelerationTest_WhenHost_Tailing_Lead_TooClose_Host_Should_Not_Accelerate(int hostMph, int hostX, int leadCarMph, int leadCarX)
        {
            double updateIntervalTotalMilliseconds = 250;
            Constants constants = new Constants();
            var SUT = Vehicle.Factory.Create("host car", hostMph, hostX, 1, true);
            var lead = Vehicle.Factory.Create("lead car", leadCarMph, leadCarX, 1, true);
            var accelerate = SUT.CalculateVehicleBrakingForceToMaintainLeadPreference(lead, updateIntervalTotalMilliseconds);
            Assert.AreEqual(0, accelerate);
        }
